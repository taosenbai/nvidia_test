import os, re, time, math, argparse, subprocess, threading
...
def build_decode_argv(codec: str, uri: str, frames: int) -> List[str]:
    if codec == "nvh264dec":
        chain = ["urisourcebin", f"uri={uri}", "!", "h264parse", "!", "nvh264dec",
                 "!", "identity", f"eos-after={frames}", "!", "fakesink", "sync=false", "qos=false"]
    elif codec == "nvh265dec":
        chain = ["urisourcebin", f"uri={uri}", "!", "h265parse", "!", "nvh265dec",
                 "!", "identity", f"eos-after={frames}", "!", "fakesink", "sync=false", "qos=false"]
    else:
        raise ValueError("decode codec 仅支持 nvh264dec/nvh265dec")
...
def build_encode_argv(codec: str, frames: int, width: int, height: int,
    ...
    if file_input:
        ...
        pre = ["videotestsrc", f"num-buffers={frames}", "is-live=false", "pattern=smpte", "!",
               "videoconvert", "!",
               (f"video/x-raw,format=NV12,width={width},height={height},framerate={framerate}"
                if use_nv12 else
    ...
    if codec == "nvh264enc":
        ...
        raise ValueError("encode codec 仅支持 nvh264enc/nvh265enc")
...
# ---------- 子进程/结果 ----------
class PipeResult:
    def __init__(self, idx: int, start_t: float):
        self.idx = idx
...
class GpuSampler:
    ...
    def _parse_lines(self, fields, out):
        ...
        for line in out.splitlines():
            ...
            if len(parts)==len(fields): rows.append(parts)
        return rows
    ...
    def _record_row(self, fields, parts):
        ...
        for f,v in zip(fields, parts):
            try:
                if f=="utilization.gpu": rec["gpu"].append(float(v))
                elif f=="utilization.encoder": rec["enc"].append(float(v))
                elif f=="utilization.decoder": rec["dec"].append(float(v))
                elif f=="memory.used": rec["mem_used"].append(float(v))
                elif f=="memory.total": rec["mem_total"].append(float(v))
            except: pass
...
def agg(vals: List[float], mode: str)->Optional[float]:
    ...
    if mode=="avg": return sum(vals)/len(vals)
    if mode=="max": return max(vals)
    if mode=="p95":
        s=sorted(vals); k=max(0, min(len(s)-1, int(math.ceil(0.95*len(s))-1)))
        return s[k]
    return sum(vals)/len(vals)

def pick_busy_gpu(samples: Dict[int, Dict[str, List[float]]], preferred_field: str, mode: str, fixed_idx: Optional[int]):
    field = preferred_field if any(samples.get(i,{}).get(preferred_field) for i in samples) else "gpu"
    if not any(samples.get(i,{}).get(field) for i in samples): return None,None,field
    if fixed_idx is not None and fixed_idx in samples:
        return fixed_idx, agg(samples[fixed_idx].get(field,[]), mode), field
    best_idx,best_val=None,None
    for i,rec in samples.items():
        v=agg(rec.get(field,[]),mode)
        if v is None: continue
        if best_val is None or v>best_val: best_idx,best_val=i,v
    return best_idx,best_val,field

# ---------- 批次运行 ----------
def run_batch(args, per_pipe_frames: int):
    ...
    for i,p in enumerate(procs):
        try:
            if deadline is None:
                ...
                ret=p.wait(timeout=max(0.0, deadline-time.time()))
            ...
            results[i].end_t = time.time()
            if results[i].start_t and results[i].end_t:
                results[i].duration_s = max(1e-6, results[i].end_t - results[i].start_t)
                results[i].fps = per_pipe_frames / results[i].duration_s

    for t in readers: t.join(timeout=1.0)

    # GPU 汇总
    gpu_info = {"sampling": gpu_sampling, "busy_idx": None, "metric": None, "value": None,
                "threshold": args.gpu_threshold, "saturated": False}
    if gpu_sampling:
        sampler.stop()
        pref = "dec" if args.codec.endswith("dec") else "enc"
        busy_idx, val, metric = pick_busy_gpu(sampler.samples, pref, args.gpu_agg, args.gpu_index)
        gpu_info.update({"busy_idx": busy_idx, "metric": metric, "value": val})
        if args.use_gpu_threshold and (val is not None) and (val >= args.gpu_threshold):
    ...
    for r in results:
        ...
        if args.require_eos: cond = cond and r.saw_eos
        if args.min_fps_per_pipe>0 and (r.fps is not None) and (r.fps < args.min_fps_per_pipe):
            cond = False
            if len(r.err_lines)<6:
                r.err_lines.append(f"Throughput below threshold: {r.fps:.2f} < {args.min_fps_per_pipe} fps")
...
# ---------- 主程序 ----------
def main():
    ap = argparse.ArgumentParser(description="NVCodec 并发上限探测（错误+吞吐阈值+GPU饱和）")
    ...
    ap.add_argument("--gpu-sample-interval", type=float, default=0.2,
                    help="GPU 采样间隔秒，默认0.2")

    ap.add_argument("--search", action="store_true", help="自动搜索最大并发")
    ...
    if args.search:
        ...
        if high > args.search_max and low == args.search_max:
            print(f"在上界 {args.search_max} 内均达标，最大并发 >= {args.search_max}")
            return
        if low == 0 and not ok:
            print("并发 1 即失败，请检查插件/驱动/管线/输入。"); return
    ...
    for r in results:
        if r.fps is not None:
            print(f"- 管线#{r.idx}: {r.fps:.2f} fps, duration={r.duration_s:.3f}s, EOS={r.saw_eos}, exit={r.exit_code}, ok={r.ok}")
    ...
    if fail or (args.use_gpu_threshold and gpu["saturated"]) or (args.min_fps_per_pipe>0 and any((r.fps or 0)<args.min_fps_per_pipe for r in results)):
        for r in results:
            if not (r.ok and r.exit_code==0 and (r.saw_eos or not args.require_eos)):
                print(f"- 失败 #{r.idx}: exit={r.exit_code}, EOS={r.saw_eos}, err={r.err_lines[:3]}")
